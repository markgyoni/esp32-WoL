...
while True:
    ...
    try:
        body = requests.get(f'https://api.telegram.org/{env.TELEGRAM_BOT_TOKEN}/getUpdates?offset={offset + 1}').json()
    except:
    ...
    for update in updates:
        ...
        if update['message']['from']['id'] in authorized and update['message']['text'] == '/wol':
            try:
                send_wol_packet(target_mac_address)

                requests.get(f'https://api.telegram.org/{env.TELEGRAM_BOT_TOKEN}/sendMessage?chat_id={update["message"]["from"]["id"]}&text=Sent%20WoL%20packet.')
            except:
